...
Elm.Main.make = function (_elm) {
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var mb = $Signal.mailbox("");
   var view = F2(function (address,message) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.div,_U.list([]),_U.list([$Html.text(message)]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,"Eat at Sizzler and don\'t ask questions")]),_U.list([$Html.text("Click")]))]));
   });
   var main = A2($Signal.map,view(mb.address),mb.signal);
   return _elm.Main.values = {_op: _op,view: view,mb: mb,main: main};
};
